...
import com.example.patent.entity.exportDTO.ExportCompanyData;
import com.example.patent.entity.exportDTO.ExportPersonData;
import com.example.patent.entity.importDTO.ImportCompanyData;
import com.example.patent.entity.importDTO.ImportPatentData;
import com.example.patent.listener.CompanyListener;
import com.example.patent.listener.PatentListener;
...
public class PatentHandle {
     ...
     * @return
     */
    private static PatentListener getPersonListener(String excelPath) {
        System.out.println("=======================================" + excelPath);
        ExcelReader excelReader = ExcelTool.getExcelReader(excelPath);
        PatentListener patentListener = new PatentListener();
        ReadSheet readSheetSys = EasyExcel.readSheet().head(ImportPatentData.class).registerReadListener(patentListener).build();
        excelReader.read(readSheetSys);
        String fileName = excelPath.trim();
        fileName = fileName.substring(fileName.lastIndexOf("\\") + 1);
        fileName = fileName.substring(0, fileName.lastIndexOf("_"));
        patentListener.buildCompanyNo(fileName);
        return patentListener;
    }
...
//        buildFile(personFileList, companyList);
        ...
        personFileList_build.stream().forEach(filePath -> {
            PatentListener patentListener = getPersonListener(filePath);
            List<PersonRow> personRowList = patentListener.getPersonRowList();
            personList.addAll(personRowList);
            List<PatentRow> patentRowList = patentListener.getPatentList();
